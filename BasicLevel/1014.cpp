...
int main()
{
    ...
    map<char, string> day, hour;
    map<int, string> minute;
    day['A'] = "MON", day['B'] = "TUE", day['C'] = "WED", day['D'] = "THU", day['E'] = "FRI", day['F'] = "SAT", day['G'] = "SUN";

    for (int i = 0; i <= 9; i++)
    {
        hour[i] = to_string(0) + to_string(i);
    }
    for (char i = 'A'; i <= 'N'; i++)
    {
        hour[i] = to_string(i - 'A' + 10);
    }
    ...
    for (int i = 0; i < 60; i++)
    {
        if (i < 10)
        {
            minute[i] = "0" + to_string(i);
        }
        else
        {
            minute[i] = to_string(i);
        }
    }

    cout << day[v1[0]] << " " << hour[v1[1]] << ":" << minute[v2[0]];
